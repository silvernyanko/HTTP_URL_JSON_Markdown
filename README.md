# HTTP_URL_JSON_Markdown

## プロダクト概要

>1. HTTPにまつわる用語についてまとめる。
> - URL
> - クエリ文字列
> - パス変数（パスパラメーター）
> - クエリ文字列とパス変数の違い
...
- ネット記事のコピペにならないように気を付けた。そのために、分からない用語一つにつき、最低3つは記事の内容の比較をした。
- クエリ文字列をクエリパラメータ、パス変数をパス変数といったような別の名称があったので、呼称を統一する、あるいはカッコ内に記すなど、振り返りやすいようにした。
- 文章だけで理解しづらかった用語は図や表、具体例、比喩などを用いて説明した。
- 始めはメモ帳に書き起こしていたが、途中でVisual Studio Codeにプレビュー機能がある事に気づき、Markdownに馴染めるようになった。
